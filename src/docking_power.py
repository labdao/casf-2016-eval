...
#	 	dfsorted=testdf.sort_values(by=['score','rmsd'],ascending=[False,True])
		...
		dockresults.ix[tmp]['Rank1']=''.join(dfsorted[0:1]['#code'])
		dockresults.ix[tmp]['RMSD1']=float(dfsorted[0:1]['rmsd'])
		dockresults.ix[tmp]['Rank2']=''.join(dfsorted[1:2]['#code'])
                dockresults.ix[tmp]['RMSD2']=float(dfsorted[1:2]['rmsd'])
		dockresults.ix[tmp]['Rank3']=''.join(dfsorted[2:3]['#code'])
                dockresults.ix[tmp]['RMSD3']=float(dfsorted[2:3]['rmsd'])
		dockresults.ix[tmp]['code']=i
		tmp+=1
		...
		for s in np.arange(2,11):
			...
			if float(sptemp.shape[0]) >= 5:
				sp.ix[i]['spearman']=np.negative(sptemp.corr('spearman')['rmsd']['score'])
			else:
				continue
...
#                dfsorted=testdf.sort_values(['score','rmsd'],ascending=[True,True])
		...
		for s in np.arange(2,11):
                        ...
                        sp=locals()[varname2]
			if float(sptemp.shape[0]) >= 5:
                        	sp.ix[i]['spearman']=sptemp.corr('spearman')['rmsd']['score']
			else:
				continue
